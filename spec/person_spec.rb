require './lib/person'

   describe Person do
#     books = [{title: 'Managment101', author: 'Rich' }, { title: 'Cooking101', author: 'Fat'}]
#
#     it 'constract of empty array at start' do
#    expect(subject.portfolio).to eq []
#  end
#
#  context 'adding book to portfolio' do
#    before { allow(library).to receive(:check_out).and_return({ }) }
#
#    it 'searched by :title' do
#      subject.rent(library, title: 'Cooking101')
#      expect(subject.portfolio).to eq [{ title: 'Cooking101', author: 'Fat' }]
#    end
#
#    it 'searched by :author' do
#      subject.rent(library, author: 'Rich')
#      expect(subject.portfolio).to eq [{ title: 'Managment101', author: 'Rich' }]
#    end
#  end
end
